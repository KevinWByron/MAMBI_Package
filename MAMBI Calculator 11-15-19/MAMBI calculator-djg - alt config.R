...
MAMBI.DJG.alt<-function(BenthicData, EG_File_Name="Ref - EG Values 2018.csv", EG_Scheme="Hybrid")
  {
  require(tidyverse)
  require(reshape2)
  require(vegan)
...
Input_File2<-Input_File %>% filter(!is.na(SalZone))
...
# After you source all the files, you can run the lines below. In case you have trouble with the benthic_query.R
# file, I've added an RData file and csv file of the output so you can run the script with those results.

#test = MAMBI.DJG.alt(benthic_data, EG_File_Name="Ref - EG Values 2018.csv", EG_Scheme="Hybrid")
